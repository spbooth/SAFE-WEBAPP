...
public class SetInput<T> extends ParseAbstractInput<String> implements ListInput<String, T> {
    ...
    public SetInput(){
    	addValidator(new FieldValidator<String>() {
			...
			public void validate(String value) throws FieldException {
				if( value != null && ! data.containsKey(mapTag(value))){
     ...
     * 
     * @param value   key value generated by input
     * @param label test presented to the user
     * @param item  item associated with key.
     */
    public void addChoice(String value,String label, T item) {
    	String mtag=mapTag(value);
    	data.put(mtag, item);
    	labels.put(item, label);
    	tags.put(item, mtag);
    	values.put(mtag, value);
    }
