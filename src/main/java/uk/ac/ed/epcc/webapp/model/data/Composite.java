...
public abstract class Composite<BDO extends DataObject, X > implements Contexed, TableStructureContributer<BDO>{
	...
	/** Extension point called after the Context is set in the parent factory
	 * (after automatic table generation
	 * 
	 */
	protected void postSetContext() {
		
	}
	/** Convenience method to register a {@link TypeProducer} generated by this {@link Composite}.
	 * Usually called from {@link #postSetContext()}
	 * 
	 * @param <C>
	 * @param <D>
	 * @param producer
	 */
	protected <C,D> void registerTypeProducer(TypeProducer<C,D> producer) {
		getRepository().addTypeProducer(producer);
	}
