...
/** {@link SQLValue} that converts an {@link SQLValue} of a different type to an integer using ceiling
...
public class CeilSQLValue<T>  implements NestedSQLValue<Integer,T> {
	private SQLValue<T> a;
    public CeilSQLValue(SQLValue<T> a){
    	this.a = a;
    }
	...
	public Integer makeObject(ResultSet rs, int pos) throws DataException, SQLException {
		...
		if( temp != null ){
	    	if( temp instanceof Number ){
	    		...
	    		return Integer.valueOf((int) Math.ceil(((Number) temp).doubleValue()));
	    	}
	    	if( temp instanceof String){
	    		return Integer.valueOf((int) Math.ceil(Double.parseDouble((String)temp)));
	    	}
	    }
		return null;
	}
